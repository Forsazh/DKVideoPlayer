...
public final class PlayerUtils {
    ...
    public static int getNetworkType(Context context) {
        ...
        if (connectMgr == null) return NO_NETWORK;
        NetworkInfo networkInfo = connectMgr.getActiveNetworkInfo();
        if (networkInfo == null) return NO_NETWORK;
        if (!networkInfo.isConnected()) return NETWORK_CLOSED;
        if (networkInfo.getType() == ConnectivityManager.TYPE_ETHERNET) return NETWORK_ETHERNET;
        else if (networkInfo.getType() == ConnectivityManager.TYPE_WIFI) return NETWORK_WIFI;
